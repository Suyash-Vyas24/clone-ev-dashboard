...
const mockLogs = [
  { id: 'L092', time: '10:42 AM', event: 'OTA Update', desc: 'Firmware v4.2.1 downloaded', status: 'Success' },
  { id: 'L093', time: '10:15 AM', event: 'Cell Balancing', desc: 'Active balancing on Block 4', status: 'Sync' },
  { id: 'L094', time: '09:30 AM', event: 'Thermal Warning', desc: 'Cooling fluid pressure drop', status: 'Warning' },
  { id: 'L095', time: '08:00 AM', event: 'Charging Session', desc: 'Fast DC Charge ended at 82%', status: 'Success' },
];

export function ActivityLogs() {
  const [filter, setFilter] = useState<'All' | 'Alerts' | 'Charging'>('All');
          ...
          <tbody className="divide-y divide-slate-700/50">
            {mockLogs.map((log) => (
              <tr key={log.id} className="hover:bg-slate-800/50 transition-colors group">
                <td className="px-6 py-4 whitespace-nowrap text-slate-300 font-mono text-xs">{log.time}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-full ring-1 ${
                    ...
                    log.status === 'Warning' ? 'bg-yellow-500/10 text-yellow-400 ring-yellow-500/20' :
